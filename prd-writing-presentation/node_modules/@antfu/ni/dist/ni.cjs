...
const index = require('./shared/ni.c583755a.cjs');
  ...
  try {
    ...
    return result.objects.map(({ package: pkg }) => ({
      title: index.formatPackageWithUrl(
        `${pkg.name.padEnd(30, " ")} ${index.$.blue(`v${pkg.version}`)}`,
...
index.runCli(async (agent, args, ctx) => {
  ...
  if (isInteractive) {
    ...
    const { mode } = await index.prompts({
      type: "select",
      name: "mode",
      message: `install ${index.$.yellow(dependency.name)} as`,
      choices: [
        {
          title: "prod",
          value: "",
